...
namespace SimpleHTTP
{
	...
	FString ToSimpleHTTPVerbTypeString(ESimpleHTTPVerbType InType)
	{
		FString NumFullName = UEnum::GetValueAsName(InType).ToString();

		int32 ProtocolHeardLen = FCString::Strlen(TEXT("ESimpleHTTPVerbType::SIMPLE_"));
		FString HandleName = NumFullName.RightChop(ProtocolHeardLen);

		return HandleName.ToLower();
	}

	ESimpleHTTPVerbType ToSimpleHTTPVerbTypeByName(const FString& InType)
	{
		FString NumFullName = *FString::Printf(TEXT("ESimpleHTTPVerbType::SIMPLE_%s"), *InType).ToUpper();

		return (ESimpleHTTPVerbType)UEnum::LookupEnumName(FName(), *NumFullName);
	}
}