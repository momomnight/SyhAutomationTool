...
bool FSimpleOSSManage::FOSS::SetBucketReferer(const FString &InBucketName, const TArray<FString> &URLReferer, bool bAllowEmptyRefere /*= false*/)
{
	...
	//先存储为string
	std::vector<std::string> RefererString;
	for (const auto& Tmp : URLReferer)
	{
		RefererString.push_back(TCHAR_TO_UTF8(*Tmp));
	}

	aos_list_init(&referer_config.referer_list);
	for (const auto &Tmp : RefererString)
	{
		//Eam "http://renzhai.net"
		oss_create_and_add_refer(pool, &referer_config,const_cast<char*>(Tmp.c_str()));
	}
...
{
	//字符串
	return GetObjectToMemory(
		InBucketName,
		InObjectName,
		Range, 
		InProgressCallback,
		StartCallback,
		[&](ANSICHAR* Buff, int32 BuffSize)
		{
			if (InProgressCallback)
			{
				EndCallback(FString(UTF8_TO_TCHAR(Buff)));
			}
			else
			{
				ContextString = UTF8_TO_TCHAR(Buff);
			}
		});
}
...
bool FSimpleOSSManage::FOSS::GetObjectToMemory(
	...
	OnceStringCallBack StartCallback,
	TFunction<void(ANSICHAR*, int32)> Fun)
{
	...
	OnceByteCallBack EndCallback)
{
	...
	[&](ANSICHAR* Buff, int32 BuffSize)
	{
		if (InProgressCallback)
		{
			EndCallback(Buff, BuffSize);
		}
		else
		{
			OutData.SetNum(BuffSize);
			FMemory::Memcpy(OutData.GetData(), Buff, BuffSize);
		}
	});
}
...
bool FSimpleOSSManage::FOSS::ListMultipartUploads(const FString &InBucketName, TArray<FMultipartPartsUploadsInfo> &MultipartPartsInfo, int32 MaxParts /*= 1000*/)
{
	...
	int32 size = 0;
	ANSICHAR *line = NULL;
	ANSICHAR* prefix = NULL;
	ANSICHAR* nextMarker = NULL;
	...
	const TMap<FString, FString> &OSSMeta /*= TMap<FString, FString>()*/)
{
	...
	[InBucketName,InLocalPaths,InObjectName,InProgressCallback, OSSMeta, this]()
	{
		this->PutObject(
			InBucketName,
			InLocalPaths,
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[URL,InLocalPaths,InProgressCallback, OSSMeta, this]()
	{
		this->PutObjectByURL(
			URL,
			InLocalPaths,
...
void FSimpleOSSManage::FOSS::AsyncPutObjectByMemory(
	const FString &InBucketName, 
	const FString &InObjectName /*= FString()*/,
	const FString &Data, 
	ProgressCallback InProgressCallback /*= nullptr*/, 
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[InBucketName,InObjectName,Data,InProgressCallback, OSSMeta, this]()
	{
		this->PutObjectByMemory(
			InBucketName,
			InObjectName,
			Data,
			InProgressCallback,
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[URL, Data, InProgressCallback, OSSMeta, this]()
	{
		this->PutObjectMemoryByURL(
			URL,
			Data,
			OSSMeta,
			InProgressCallback);
...
void FSimpleOSSManage::FOSS::AsyncPutObjectMemoryByURL(
	const FString& URL, 
	const TArray<uint8>& Data, 
	const TMap<FString, FString>& OSSMeta, 
	ProgressCallback InProgressCallback)
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[InBucketName,InObjectName,InUploadFilePath,PartSize,InProgressCallback,OSSMeta,this]()
	{
		this->ResumableUploadObject(
			InBucketName,
			InObjectName,
			InUploadFilePath,
			PartSize,
	...
	CallbackUploadPart InCallbackUploadPart)
{
	...
	while (pos < file_length)
	{
		...
		aos_str_set(&upload_file->filename, Tmp_file.c_str());
	...
	const TMap<FString, FString> &OSSMeta)
{
	...
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[pool,oss_client_options,InUploadId,InBucketName,InObjectName,NewLocalPaths,PartSize,InCallbackUploadPart,this]()
	{
		this->UploadPart_Internal(
			pool, 
			oss_client_options, 
			InUploadId, 
			InBucketName, 
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[InBucketName,InObjectName, InLocalPaths, Range,InProgressCallback,this]()
	{
		this->GetObjectToLocal(
			InBucketName,
			InObjectName,
			InLocalPaths,
			Range,
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[URL,Range,InLocalPaths,InProgressCallback,InStartCallback,InEndCallback,this]()
	{
		this->GetObjectToLocalByURL(
			URL,
			InLocalPaths,
			Range,
			InProgressCallback,
...
void FSimpleOSSManage::FOSS::AsyncGetObjectToMemory(
	const FString &InBucketName,
	const FString &InObjectName, 
	FRange Range /*= FRange()*/,
	ProgressCallback InProgressCallback /*= nullptr*/, OnceStringCallBack InStartCallback /*= NULL*/, OnceStringCallBack InEndCallback /*= NULL*/)
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[InBucketName,InObjectName,Range,InProgressCallback,InStartCallback,InEndCallback,this]()
	{
		FString ContextString;
		this->GetObjectToMemory(
			InBucketName,
			InObjectName,
			ContextString,
			Range,
...
void FSimpleOSSManage::FOSS::AsyncGetObjectToMemory(
	...
	ProgressCallback InProgressCallback, 
	OnceStringCallBack InStartCallback, 
	OnceByteCallBack InEndCallback)
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[InBucketName,InObjectName,Range,InProgressCallback,InStartCallback,InEndCallback,this]()
	{
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[URL,Range,InProgressCallback,InStartCallback,InEndCallback,this]()
	{
		FString ContextString;
		this->GetObjectToMemoryByURL(
			URL,
			ContextString,
			Range,
			InProgressCallback,
...
void FSimpleOSSManage::FOSS::AsyncGetObjectToMemoryByURL(
	const FString& URL, 
	FRange Range, 
	ProgressCallback InProgressCallback, 
	OnceStringCallBack InStartCallback, 
...
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask,
	[InBucketName, InObjectName, PartSize, InDownloadFilePath,InProgressCallback,this]()
	{
		this->ResumableDownloadObject(
			InBucketName,
			InObjectName,
			InDownloadFilePath,
			PartSize,
	...
	const TMap<FString, FString> &OSSMeta /*= TMap<FString, FString>()*/)
{
	...
	return PutObjectByMemory(InBucketName, InObjectName, Object_Data.c_str(),strlen(Object_Data.c_str()), InProgressCallback, OSSMeta);
}

bool FSimpleOSSManage::FOSS::PutObjectByMemory(const FString& InBucketName, const FString& InObjectName, const TArray<uint8>& InData, ProgressCallback InProgressCallback, const TMap<FString, FString>& OSSMeta)
{
	return PutObjectByMemory(InBucketName, InObjectName, InData.GetData(),InData.Num(), InProgressCallback, OSSMeta);;
}

bool FSimpleOSSManage::FOSS::PutObjectByMemory(const FString& InBucketName, const FString& InObjectName,const void* InData, int32 InSize, ProgressCallback InProgressCallback, const TMap<FString, FString>& OSSMeta)
{
	STRING_TO_AOS_STRING_T(bucket, InBucketName);
	STRING_TO_AOS_STRING_T(object, InObjectName);
	CREATE_CONFIGURATION_AND_STORAGE;
	...
	aos_list_init(&buffer);
	content = aos_buf_pack(oss_client_options->pool,InData,InSize);
...
{	
	...
	return PutObjectMemoryByURL(URL, Object_Data.c_str(), strlen(Object_Data.c_str()), OSSMeta, InProgressCallback);
}

bool FSimpleOSSManage::FOSS::PutObjectMemoryByURL(const FString& URL, const TArray<uint8>& Data, const TMap<FString, FString>& OSSMeta, ProgressCallback InProgressCallback)
{
	return PutObjectMemoryByURL(URL, Data.GetData(), Data.Num(), OSSMeta, InProgressCallback);
}

bool FSimpleOSSManage::FOSS::PutObjectMemoryByURL(const FString& URL, const void* Data, int32 DataSize, const TMap<FString, FString>& OSSMeta, ProgressCallback InProgressCallback)
{
	CREATE_CONFIGURATION_AND_STORAGE;
	STRING_TO_AOS_STRING_T(InUrl, URL);
	...
	aos_list_t* resp_body = nullptr;
	aos_list_init(&buffer);
	content = aos_buf_pack(oss_client_options->pool, Data, DataSize);
	aos_list_add_tail(&content->node, &buffer);
