...
std::string ObjectName##_name = string_To_UTF8(WstringToString(std::wstring(*InObjectName))); \
/*Assign data of type char * to the storage space of type AOS ﹣ string ﹣ t */ \
aos_str_set(&ObjectName, ObjectName##_name.c_str());

#define UTF8_STRING_TO_AOS_STRING_T(ObjectName,InObjectName) \
aos_string_t ObjectName;\
std::string Tmp_##ObjectName = string_To_UTF8(WstringToString(std::wstring(*InObjectName))); \
/*Utf8 code the multi byte Chinese name.*/ \
/*ANSICHAR InObjectName##buf[1024];*/\
/*ANSICHAR *Tmp_##ObjectName = multibyte_to_utf8(InObjectName##_Char.c_str(), InObjectName##buf, 1024);*/\
/*Assign data of type char * to the storage space of type AOS ﹣ string ﹣ t */  \
aos_str_set(&ObjectName, Tmp_##ObjectName.c_str());
